...
def calc_error(y,y_hat,method='MAE', normalized=False):
  # calculates the error of predicted vs true values
  # y true values
  # y_hat predicted
...
def normalize_error(y,metric,method):
  # function that normalizes error
  # y = true values
  # metric = calculated error
  # method = avarage, range, iqr (of the true data)
  if method == 'average':
...
def run_arima_get_rmse(merchant, merchant_name, train_test_split = 21, seasonality = 12, D_val =1):
  ...
  # to normalize the error run the line below instead
